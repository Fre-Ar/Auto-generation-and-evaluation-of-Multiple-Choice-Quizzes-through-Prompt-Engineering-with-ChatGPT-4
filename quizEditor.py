...
def saveQuiz(quizData, filename='edited_quiz.json'):
...
def editQuiz(quizData):
    ...
    saveButton = widgets.Button(description="Save Changes", button_style='success')  # Button to save the quiz
    
    # Updates quiz title
    def updateTitle(change):
        quizData['title'] = change['new']
    # Updates section randimization toggle
    def updateSectionOrderRandomized(change):
        quizData['sectionOrderRandomized'] = change['new']
    # callback function for the save changes button
...
def createSectionEditor(section, sections, accordion):
    nameInput = widgets.Text(value=section['name'], description='Section Name:')
    randomizeQuestions = widgets.Checkbox(value=section['questionOrderRandomized'], description='Randomize Question Order')
    questionsVBox = widgets.VBox([createQuestionEditor(question, section['questions']) for question in section['questions']])
    addQuestionBtn = widgets.Button(description="Add New Question")
    removeSectionBtn = widgets.Button(description="Remove This Section", button_style='danger')
    ...
    def addQuestion(b):
        ...
        questionsVBox.children = tuple(list(questionsVBox.children) + [createQuestionEditor(newQuestion, section['questions'])])
...
def createQuestionEditor(question, questions):
    contextInput = widgets.Text(value=question['context'], description='Context:')
    questionInput = widgets.Text(value=question['question'], description='Question:')
...
def createOptionEditor(option, options):
    optionInput = widgets.Text(value=option['option'], description='Option:')
    ...
    feedbackInput = widgets.Text(value=option['feedback'], description='Feedback:')
